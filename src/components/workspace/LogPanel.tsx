...
export function LogPanel({
  ...
  const renderLogDetails = (log: ProjectLog) => {
    ...
    if (actUpper.includes('CREATE SPEC')) {
      return (
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[10px] text-slate-400">Title: "{details.title || 'Untitled'}"</p>
        </div>
      );
    }
    ...
    if (actUpper.includes('EDIT SPEC')) {
      return (
        <div className="flex flex-col gap-1 text-xs">
          ...
          {details.title && <p className="text-[10px] text-slate-400">Title: "{details.title}"</p>}
        </div>
      );
    }
    if (actUpper.includes('ADD MEMBER')) {
      return (
        <div className="flex flex-col gap-1 text-xs">
          ...
          <p className="text-[10px] text-emerald-400 font-medium">{details.email || details.uuid || 'Unknown User'}</p>
        </div>
      );
    }
    ...
    return (
      <div className="flex flex-col gap-1 italic text-slate-400 text-[10px]">
  ...
  return (
    <div className="bg-slate-900/30 rounded-2xl border border-slate-800/50 overflow-hidden flex flex-col max-h-[500px]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {logs.map(log => (
          <div key={log.id} className="relative pl-6 pb-2 border-l border-slate-800 last:pb-0">
            <div className="absolute left-[-5px] top-1 w-2.5 h-2.5 rounded-full bg-slate-800 border-2 border-slate-950" />
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-bold text-orange-500 uppercase tracking-tight">{log.action}</span>
